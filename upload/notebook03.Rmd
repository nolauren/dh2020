...
```{r}
movie_subset <- slice_sample(data = movies, n = 15)
...
```{r}
movie_subset <- arrange(data = movies, desc(gross))
movie_subset <- slice_head(data = movie_subset, n = 10)
...
```{r}
movie_subset <- filter(data = movie, genre == "Crime")
