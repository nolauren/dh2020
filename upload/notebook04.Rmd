...
but we can quickly estimate this data using a pre-trained NLP model. The code to do
this in R is quite short and given the next block of code. The code is commented out
because it can take 20-30 minutes to finish running and we can instead load a saved
version of the data
...
representing information about the token. Specifically:
  ...
  - lemma: lemmatized version of the token (e.g., remove capitalization, makes all
    nouns singular, all verbs in infinitive form)
...
which can be punctionation or individual words.
...
Sometimes we want the data to tell us what terms are important rather than have to figure them
out ourselves. One method for doing this is term-frequency inverse document frequency (TF-IDF).
...
Another similar approach to TF-IDF that works well for this corpus is too look at all of the
...
most strongly differentiate between two sets of documents. Here, we will start by
pick two presidents and seeing what terms most strongly differ between their 
speeches. For reference, here are the "short" names of all the presidents in our
data (usually last name, except when ambiguous):
...
lda_model <- LDA(x = tf_matrix, k = 16, control = list(seed = 2811, verbose = 1))
```
...
As a final method, we will use a technique called principal component analysis to
